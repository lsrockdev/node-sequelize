...
const OrdersController = () => {
  const placeOrder = async (req, res) => {
    ...
    try {
      ...
      carts.forEach(cart => {
        ...
        const itemTotal = cart.Inventory.price * cart.qty;
        const deliveryFeeTotal = cart.Inventory.Category.deliveryFee * cart.qty;
        storeTotals[storeId][subtotal] =
          storeTotals[storeId][subtotal] + itemTotal;
        storeTotals[storeId][deliveryFeeTotal] =
          storeTotals[storeId][subtotal][deliveryFeeTotal] +
          cart.Inventory.Category.deliveryFee;
      ...
      // For each store:
      Object.keys(storeItems).forEach(async storeId => {
        ...
        await createLineItemsForOrder(storeItems[storeId]);
      });
  ...
  const createLineItemsForOrder = async lineItems => {
    let subtotal = 0;
